...
namespace pod5 {
    ...
    public:
        ...
        ReadData(
                ...
                : read_id(read_id), read_number(read_number), start_sample(start_sample), median_before(median_before),
                  end_reason(end_reason), end_reason_forced(end_reason_forced), run_info(run_info),
                  num_minknow_events(num_minknow_events), tracked_scaling_scale(tracked_scaling_scale),
                  tracked_scaling_shift(tracked_scaling_shift), predicted_scaling_scale(predicted_scaling_scale),
                  predicted_scaling_shift(predicted_scaling_shift),
                  num_reads_since_mux_change(num_reads_since_mux_change), time_since_mux_change(time_since_mux_change),
                  channel(channel), well(well), pore_type(pore_type), calibration_offset(calibration_offset),
    ...
    public:
        ...
        RunInfoData(
                ...
                : acquisition_id(acquisition_id), acquisition_start_time(acquisition_start_time), adc_max(adc_max),
                  adc_min(adc_min), context_tags(context_tags), experiment_name(experiment_name),
                  flow_cell_id(flow_cell_id), flow_cell_product_code(flow_cell_product_code),
                  protocol_name(protocol_name), protocol_run_id(protocol_run_id),
                  protocol_start_time(protocol_start_time), sample_id(sample_id), sample_rate(sample_rate),
                  sequencing_kit(sequencing_kit), sequencer_position(sequencer_position),
                  sequencer_position_type(sequencer_position_type), software(software), system_name(system_name),
                  system_type(system_type), tracking_id(tracking_id) {
        }
